...
export default function ClientTestimonials({ testimonials, homePageData }: ClientTestimonialsProps) {
  ...
  const stats: TestimonialStats = React.useMemo(() => {
    ...
    const averageRating = totalReviews > 0 
      ? publishedTestimonials.reduce((sum, t) => sum + t.rating, 0) / totalReviews 
      : 0

    const ratingDistribution = publishedTestimonials.reduce((dist, t) => {
      const rating = t.rating.toString()
      dist[rating] = (dist[rating] || 0) + 1
      return dist
    }, {} as { [key: string]: number })

    const platforms = publishedTestimonials.reduce((platforms, t) => {
      if (t.platform) {
    ...
    const averageScores = testimonialsWithScores.length > 0 ? {
      habitaciones: testimonialsWithScores.reduce((sum, t) => sum + (t.scores?.habitaciones || 0), 0) / testimonialsWithScores.length,
      servicio: testimonialsWithScores.reduce((sum, t) => sum + (t.scores?.servicio || 0), 0) / testimonialsWithScores.length,
      ubicacion: testimonialsWithScores.reduce((sum, t) => sum + (t.scores?.ubicacion || 0), 0) / testimonialsWithScores.length,
    } : undefined
  ...
  const featuredTestimonials = testimonials.filter((t) => t.featured && t.published)
  ...
  const backgroundImageUrl = homePageData?.testimonialsBackgroundImage?.url || 
    ...
    <div className='min-h-screen'>
      ...
      >
        ...
        <div className="absolute inset-0">
          <Image src={backgroundImageUrl} alt="Hotel Juan María" fill className="object-cover" />
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
            ...
            >
              ...
              <div className={`${isMobile ? '' : 'col-span-5'} flex flex-col gap-4 lg:gap-6`}>
                ...
                >
                  {homePageData?.testimonialsSubtitle || 'Descubre lo que nuestros huéspedes dicen sobre su estadía en Hotel Juan María'}
                </p>
                          ...
                          <div className="flex items-center space-x-4">
                            <div className={`${isMobile ? 'w-12 h-12' : 'w-12 h-12 md:w-16 md:h-16'} bg-gradient-to-br from-white/20 via-white/10 to-white/5 backdrop-blur-xl rounded-full flex items-center justify-center border border-white/20 overflow-hidden`}>
                              {featuredTestimonials[currentSlide]?.avatar ? (
                                <Image
                                  src={getAvatarUrl(featuredTestimonials[currentSlide].avatar) || ''}
