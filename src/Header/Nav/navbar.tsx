...
import icon from '@/public/GrayIcon.jpg'
...
export default function Navbar({ items }: NavbarProps) {
    ...
    return (
      items?.map((it: PayloadItem) => {
        ...
        const dropdownItems = dropdownItemsRaw
          ?.map((di) => ({
            name: di.link?.label || '',
            href: linkToHref(di.link) || '#',
          }))
          .filter((di) => di.name && di.href)
        const hasDropdown =
          (it as any)?.hasDropdown === true && Boolean(dropdownItems?.length)
  ...
  return (
    <>
        ...
        <div className='container mx-auto px-6 lg:px-8 relative'>
          <div className='flex items-center justify-between h-20'>
            ...
            <div className='flex items-center group relative'>
              <Link href='/' className='relative'>
                <div className='relative group'>
                  ...
                  {/* Logo para Desktop - OCULTO en móvil */}
                  <HotelIcon
                    variant='gray'
                    width={56}
                    height={56}
                    className='hidden md:block relative z-10 transition-all duration-500 group-hover:rotate-3 group-hover:scale-105 drop-shadow-lg'
                  />
                  ...
                  {item.hasDropdown && item.dropdownItems && (
                    ...
                    >
                      <div className='mt-4 bg-white rounded-2xl shadow-xl border border-gray-200/50 overflow-hidden'>
                        <div className='absolute -top-2 left-1/2 transform -translate-x-1/2 w-4 h-4 bg-white rounded-full shadow-lg border border-gray-200/50'></div>
            ...
            <div className='hidden lg:flex items-center space-x-4'>
              ...
              >
                <div
                  className={`absolute inset-0 ${gradientClasses.premium}`}
                ></div>
      ...
      {/* MOBILE MENU - Habitaciones es link directo */}
      <div
          ...
          <div className='p-4 space-y-1 max-h-[calc(100vh-200px)] overflow-y-auto'>
            {navigationItems.map((item) => (
              <div key={item.name}>
                {item.hasDropdown ? (
                  <>
                    {/* Habitaciones como link directo en mobile */}
                    <div className='flex items-center'>
                      <Link
                        href={item.href}
                    ...
                    {/* Dropdown de habitaciones específicas */}
                    <div
